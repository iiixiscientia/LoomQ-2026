...
首屏不是空白输入框，而是三幕绘本式引导，视觉上做成深色 HUD 终端风格（青色/品红双色、
切角面板、等宽字体读数）：第一幕一个轻轻闪烁的信号源，轻触后"坍缩"成青色或品红，
气泡文字用大白话解释叠加与测量，并在旁边同步展示真实的态矢量记号（如
`a|0⟩+b|1⟩, |a|²=|b|²=0.5`）；第二幕出现第二个信号源，两者之间有一条常驻的"数据链"
细线，轻触后两者总是同步坍缩，并展示贝尔态公式 `(|00⟩+|11⟩)/√2`，文案特别澄清"这不是
瞬间传信号，是关联"，避免强化常见的物理误解；走完两幕后才浮现输入框，进入自由对话
（原有三张建议卡片 + agent_chat 自然语言交互）。前两幕的坍缩结果不是预设动画——由前端
向 /api/simulate 请求本地精确态矢量模拟器算出的真实分布，再做加权随机抽样决定信号源
的颜色，因此**不占用 LLM 调用预算、也不需要配置 DeepSeek key 就能体验**，只有第三幕的
自由问答才会调用 agent_chat。自由对话阶段 agent 返回的电路自动渲染测量结果柱状图
（Chart.js CDN），关键量子概念（纠缠/叠加/GHZ）以概念卡片形式穿插解释。CLI 模式
（--cli）作为无浏览器环境的 fallback。

设计参考：视觉语言借鉴 Cyberpunk 2077 的对话框 HUD 美术（霓虹描边、切角面板、等宽
系统提示），叙事节奏与"图 + 真实公式 + 一句话人话"三件套的教学法借鉴自 Kitty Yeung
的量子计算科普漫画（尤其是其贝尔态/纠缠讲解页与"迷思 vs 事实"对照页的表达方式）：
绝不用动画糊弄公式，每一步展示的数字和 ket 记号都来自本地模拟器算出的真实结果。

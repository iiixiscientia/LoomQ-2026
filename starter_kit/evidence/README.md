...
首屏不是空白输入框，而是三幕绘本式引导：第一幕一颗轻轻脉动的小球，轻触后"坍缩"成
蓝色或橙色，气泡文字用大白话解释叠加与测量；第二幕出现第二颗球，轻触后两球总是
同色，解释量子纠缠；走完两幕后才浮现输入框，进入自由对话（原有三张建议卡片 +
agent_chat 自然语言交互）。前两幕的坍缩结果不是预设动画——由前端向 /api/simulate
请求本地精确态矢量模拟器算出的真实分布，再做加权随机抽样决定球的颜色，因此**不占用
LLM 调用预算、也不需要配置 DeepSeek key 就能体验**，只有第三幕的自由问答才会调用
agent_chat。自由对话阶段 agent 返回的电路自动渲染测量结果柱状图（Chart.js CDN），
关键量子概念（纠缠/叠加/GHZ）以概念卡片形式穿插解释。CLI 模式（--cli）作为无浏览器
环境的 fallback。
